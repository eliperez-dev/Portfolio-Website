...
impl ALU {
    ...
    pub fn execute(&mut self, registers: &Registers, instr: &Instruction, input_register: &mut i32, waiting_for_input: &mut bool) {
        ...
        // Note: Logic follows TS implementation
        // TS: "Math ops read from registers. Only IMM reads immediate."
        // Using register.read() handles R0-R7.
        // For IMM, Parser sets OperandType, but logic here assumes 'registers.read' usage for math?
        // Wait, looking at TS ALU:
        // "let b_data = registers.read(instr.b.data);"
        // If instr.b.type is Immediate, instr.b.data is the value. 
        // registers.read(val) checks if val is 0-7. If val is 10 (immediate 10), registers.read(10) returns 0 (out of bounds).
        // BUT for IMM instruction, we don't use ALU math usually?
        // TS ALU: "if (op === Operation.ADD) result = a_data + b_data;"
        // Parser creates IMM instruction with: a=Reg, b=Immediate. 
        // Wait, IMM instruction is handled in WriteBack stage in TS, NOT ALU execute?
        // TS WriteBack: "else if (op === Operation.IMM) { this.registers.write(a, b); }"
        // So ALU only handles Math/Logic/Inp.
        // For Math (ADD R1 R2), Parser sets a=R1, b=R2. registers.read(R2) works.
        // For ADD R1 5? Parser doesn't allow Immediate for ADD usually unless syntax allows.
        // TS Parser: "if (needed[1]) ... valB = this.parseOperand..."
        // TS ALU: "let b_data = registers.read(instr.b.data);" 
        // This implies B is always treated as a register index for ALU ops.
        // So `ADD R1 5` (if parsed as Reg, Imm) would read Register[5].
...
impl Parser {
    ...
    fn get_needed_operands(op: Operation, args: OperationArgs) -> (bool, bool) {
        match op {
            ...
            Operation::OUT | Operation::STORE | Operation::LOAD => (true, true),
